...
def apply_path_effect(grp, lpe):
    ...
    for one_lpe in lpe_list:
        ...
        if pe_list is None:
            ...
            obj_grp.set("inkscape:path-effect", "%s;%s" % (pe_list, str(one_lpe)))
...
def export_text_to_path(text, combine=True):
    ...
    obj_id = obj.get_id()
    with TemporaryDirectory(prefix="inkscape_command_with_actions") as tmpdir:
        svg_file = inkex.command.write_svg(svg, tmpdir, "input_for_text_to_path.svg")
...
def create_shadow_filter(
    shadow_color, opacity=0.3, stdDeviation=3.125, dx=2.33645, dy=2.33645
):
    ...
    composite1 = filter7285.add(
        "Composite", src1=flood, src2="SourceGraphic", operator="in"
    )
...
def create_box(bb, color, opacity=0.3, margin_x=0, margin_y=0):
    ...
    return rect(
        (bb.left - margin_x, bb.top - margin_y),
        (bb.right + margin_x, bb.bottom + margin_y),
        fill=color,
        opacity=opacity,
        stroke_width=0,
    )
...
parser.add_argument(
    "--font-family",
    type=str,
    default=settings["font_family"],
    help=f"字幕テキストのフォントファミリー. (デフォルト値: {settings['font_family']})",
)
parser.add_argument(
    "--font-style",
    type=str,
    default=settings["font_style"],
...
parser.add_argument(
    ...
    type=str,
    default=settings["text_anchor"],
    help=f"テキストのアンカー. (デフォルト値: {settings['text_anchor']})",
)
parser.add_argument(
    "--line-sep",
    type=float,
    default=settings["line_sep"],
...
parser.add_argument(
    ...
    help=f"テキストの色. (デフォルト値: {settings['text_color']})",
)
parser.add_argument(
    "--letter-spacing", type=float, default=None, help="文字の間隔. (デフォルト値: なし)"
)
parser.add_argument(
    "--offset-rate",
    type=float,
    default=settings["offset_rate"],
    help=f"縁取り用に拡大する割合.基準はフォントサイズ. (デフォルト値: {settings['offset_rate']})",
)
parser.add_argument(
    "--offset-color",
    type=str,
    default=settings["offset_color"],
    help=f"縁取りの色. (デフォルト値: {settings['offset_color']})",
)
parser.add_argument(
    "--offset-stroke",
    type=str,
...
parser.add_argument(
    "--offset-stroke-rate",
    type=float,
    default=settings["offset_stroke_rate"],
    help=f"縁取りの枠線の幅.基準はフォントサイズ. (デフォルト値: {settings['offset_stroke_rate']})",
)
parser.add_argument(
    "--shadow-color", type=str, default=None, help="影の色.色未指定の場合は影を表示しない. (デフォルト値: なし)"
)
parser.add_argument(
    "--box-color", type=str, default=None, help="背景色.色未指定の場合は背景を塗り潰さない. (デフォルト値: なし)"
)
parser.add_argument(
    "--export-ids", type=int, nargs="*", default=[], help="出力する字幕番号. 未指定時は全ての字幕を出力する"
)
...
if settings["shadow_color"]:
    filter = create_shadow_filter(
        settings["shadow_color"],
        settings["shadow_opacity"],
        settings["shadow_blur_stddeviation"],
        settings["shadow_dx"],
        settings["shadow_dy"],
    )
...
for item in items:
    ...
    text_group = create_text_group(
        ...
        font_size=(settings["font_size"] * pt),
        font_weight=settings["font_style"],
    ...
    if settings["box_color"]:
        ...
        box = create_box(
            ...
            margin_x=box_margin_x + margin_offset_size,
            margin_y=box_margin_y + margin_offset_size,
        )
        box.z_order("bottom")
